...
impl SqlitePendingOrderStore {
    ...
    pub async fn get_or_init_pool(&self, account_id: &str) -> Result<SqlitePool, TradeError> {
        ...
        let pool = SqlitePoolOptions::new()
            ...
            .await
            .map_err(|e| {
                TradeError::InternalError(format!("Failed to connect to SQLite: {}", e))
            })?;
    ...
    fn row_to_order(row: sqlx::sqlite::SqliteRow) -> Result<Order, TradeError> {
        ...
        let price =
            match price_str {
                Some(p) => Some(Decimal::from_str(&p).map_err(|_| {
                    TradeError::InternalError("Price decimal parse error".to_string())
                })?),
                None => None,
            };
        ...
        let volume = Decimal::from_str(&volume_str)
            .map_err(|_| TradeError::InternalError("Volume decimal parse error".to_string()))?;

        let filled_volume_str: String = row.get("filled_volume");
        let filled_volume = Decimal::from_str(&filled_volume_str).map_err(|_| {
            TradeError::InternalError("Filled Volume decimal parse error".to_string())
        })?;
...
impl PendingOrderPort for SqlitePendingOrderStore {
    ...
    async fn update_status(
        ...
        for entry in self.pools.iter() {
            let pool = entry.value();
            let res =
                sqlx::query("UPDATE pending_orders SET status = ?, updated_at = ? WHERE id = ?")
                    .bind(status_str)
                    .bind(Utc::now())
                    .bind(&order_id.0)
                    .execute(pool)
