...
use okane_core::test_utils::{MemMarketStore, MockMarketDataProvider, wait_for_condition};
...
async fn test_stock_latest_candle() -> anyhow::Result<()> {
    ...
    assert_eq!(
        stock
            .latest_candle(TimeFrame::Minute1)
            .map_err(|e| anyhow::anyhow!(e))?
            .ok_or_else(|| anyhow::anyhow!("Candle null"))?
            .close,
        dec!(105.0)
    );
    Ok(())
...
async fn test_stock_last_closed_candle() -> anyhow::Result<()> {
    ...
    assert_eq!(
        stock
            .last_closed_candle(TimeFrame::Minute1)
            .map_err(|e| anyhow::anyhow!(e))?
            .ok_or_else(|| anyhow::anyhow!("Closed candle null"))?
            .close,
        dec!(108.0)
    );
    Ok(())
...
async fn test_stock_subscribe() -> anyhow::Result<()> {
    ...
    assert_eq!(
        received
            .ok_or_else(|| anyhow::anyhow!("Received null"))??
            .close,
        dec!(99.0)
    );
    Ok(())
...
async fn test_stock_fetch_history() -> anyhow::Result<()> {
    ...
    let history = stock
        .fetch_history(TimeFrame::Day1, start, end)
        .await
        .map_err(|e| anyhow::anyhow!(e))?;
