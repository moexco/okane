...
async fn test_market_with_real_yahoo_feed() -> anyhow::Result<()> {
    ...
    let store = Arc::new(SqliteMarketStore::new().map_err(|e| anyhow::anyhow!("Failed to create real store: {}", e))?);
    ...
    match first_candle {
        Ok(Some(candle_res)) => {
            ...
            // 6. 验证聚合根的同步快照是否已更新
            assert!(stock.current_price().map_err(|e| anyhow::anyhow!(e))?.is_some());
            assert!(stock.latest_candle(TimeFrame::Minute1).map_err(|e| anyhow::anyhow!(e))?.is_some());
        }
...
async fn test_market_broadcast_with_real_feed() -> anyhow::Result<()> {
    ...
    // 获取同一个聚合根的两个引用
    let stock_a = market.get_stock(symbol).await.map_err(|e| anyhow::anyhow!("A failed: {}", e))?;
    let stock_b = market.get_stock(symbol).await.map_err(|e| anyhow::anyhow!("B failed: {}", e))?;

    // 同时订阅
    let mut stream_a = stock_a.subscribe(TimeFrame::Minute1).map_err(|e| anyhow::anyhow!(e))?;
    let mut stream_b = stock_b.subscribe(TimeFrame::Minute1).map_err(|e| anyhow::anyhow!(e))?;
