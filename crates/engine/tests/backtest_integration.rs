...
async fn test_end_to_end_backtest_with_time_travel() -> anyhow::Result<()> {
    let base_time = Utc
        .with_ymd_and_hms(2025, 1, 1, 9, 30, 0)
        .single()
    ...
    account_manager.ensure_account_exists(
        account_id.clone(),
        Decimal::from_str_exact("10000.0").map_err(|e| anyhow::anyhow!(e))?,
    );
    ...
    // 运行整个回测
    driver
        .run("VOO", TimeFrame::Minute1, base_time, 3)
        .await
        .map_err(|e| anyhow::anyhow!(e))?;

    // 检查最终账户流水状态
    let snapshot = account_manager
        .snapshot(&account_id)
        .await
        .map_err(|e| anyhow::anyhow!(e))?;
