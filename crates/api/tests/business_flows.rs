...
use okane_api::types::{
    ApiResponse, ChangePasswordRequest, CreateAccountRequest, CreateUserRequest, LoginRequest,
    LoginResponse, StartStrategyRequest, StrategyResponse,
};
...
async fn test_auth_and_password_flow() -> anyhow::Result<()> {
    ...
    let mut admin = store
        .get_user("admin")
        .await?
        .ok_or_else(|| anyhow::anyhow!("Admin missing"))?;
    ...
    let login_data: ApiResponse<LoginResponse> = res
        .json()
        .await
        .map_err(|e| anyhow::anyhow!("Failed to parse login response: {}", e))?;
    let admin_token = login_data
        .data
        .ok_or_else(|| anyhow::anyhow!("Login response missing data"))?
        .access_token;
    ...
    assert_eq!(
        res.status(),
        StatusCode::UNAUTHORIZED,
        "Old password should be invalid after successful change"
    );
...
async fn test_admin_user_management() -> anyhow::Result<()> {
    ...
    let admin_token = res
        ...
        .data
        .ok_or_else(|| anyhow::anyhow!("Admin token null"))?
        .access_token;
...
async fn test_user_account_and_strategy_deployment() -> anyhow::Result<()> {
    ...
    let admin_token = res
        ...
        .map_err(|e| anyhow::anyhow!(e))?
        .data
        .ok_or_else(|| anyhow::anyhow!("Admin data null"))?
        .access_token;
    ...
    let trader_token = res
        ...
        .map_err(|e| anyhow::anyhow!(e))?
        .data
        .ok_or_else(|| anyhow::anyhow!("Trader data null"))?
        .access_token;
    ...
    let strategy_id = deploy_res
        .json::<ApiResponse<String>>()
        .await
        .map_err(|e| anyhow::anyhow!(e))?
        .data
        .ok_or_else(|| anyhow::anyhow!("Strategy ID null"))?;

    // 5. 验证列表
    let list_res = assert_get!(
        &client,
        format!("{}/api/v1/user/strategies", base_url),
        Some(&trader_token),
        StatusCode::OK
    );
    let strategies_page = list_res
        .json::<ApiResponse<okane_api::types::Page<StrategyResponse>>>()
        .await
        .map_err(|e| anyhow::anyhow!(e))?
        .data
        .ok_or_else(|| anyhow::anyhow!("Strategies null"))?;
...
async fn test_strategy_backtest() -> anyhow::Result<()> {
    ...
    let token = user_data
        .data
        .ok_or_else(|| anyhow::anyhow!("User data null"))?
        .access_token;
    ...
    let token = token_data
        .data
        .ok_or_else(|| anyhow::anyhow!("Token data null"))?
        .access_token;

    // 回测逻辑
    let js_code =
        "function onInit() {} function onCandle(input) { host.buy('AAPL', '150.0', '10'); }";
    let end_time = chrono::Utc::now();
    let start_time = end_time - chrono::Duration::days(1);

    let res = assert_post!(
        &client,
        format!("{}/api/v1/user/backtest", base_url),
        Some(&token),
        &serde_json::json!({
            "symbol": "AAPL",
            "timeframe": "1d",
    ...
    let bt_data = res
        .json::<ApiResponse<okane_api::types::BacktestResponse>>()
        .await
        .map_err(|e| anyhow::anyhow!(e))?;
    let result = bt_data
        .data
