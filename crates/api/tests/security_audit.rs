...
async fn test_refresh_token_rotation_and_reuse_detection() -> anyhow::Result<()> {
    ...
    let tokens = login_data.data.ok_or_else(|| anyhow::anyhow!("login response missing tokens"))?;
    ...
    let tokens2 = refresh_data1.data.ok_or_else(|| anyhow::anyhow!("refresh 1 missing tokens"))?;
    ...
    let tokens3 = refresh_data2.data.ok_or_else(|| anyhow::anyhow!("refresh 2 missing tokens"))?;
    ...
    assert!(error_body.error.as_ref().map(|e| e.contains("reuse detected")).unwrap_or(false), "error should mention reuse detection");
...
async fn test_password_change_revokes_all_sessions() -> anyhow::Result<()> {
    ...
    let tokens = res.json::<ApiResponse<LoginResponse>>().await?.data.ok_or_else(|| anyhow::anyhow!("login missing tokens"))?;
